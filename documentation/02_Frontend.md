...
## 12. Avantages, limites et pistes d'amélioration (observations réelles)

### 12.1 Avantages observés
- **Cohérence visuelle forte** : tous les composants décideurs partagent les mêmes motifs (cartes `card-3d`, dégradés glow, variables de thème), donnant une identité visuelle homogène malgré l'absence de design system formel.
- **Authentification JWT et RBAC fonctionnels** pour l'espace décideurs, avec gestion d'expiration de session (côté serveur via `exp`) et garde de route (`ProtectedRoute`).
- **Export PDF entièrement côté client**, sans dépendance serveur supplémentaire — pratique pour la portabilité du projet.
- **Thème clair/sombre persistant et appliqué dynamiquement** via variables CSS, plutôt qu'un simple `className` conditionnel.
- **Séparation claire entre données réelles et données simulées**, explicitement documentée dans le code (`evolutionData.js`) et dans l'UI elle-même (bandeau méthodologique de `EvolutionTemporelle.jsx`).

### 12.2 Limites concrètes (preuves dans le code)
- **Duplication massive de la constante API** : `http://localhost:8000` est répété en dur dans au moins 9 fichiers différents (`App.js`, `AvisCitoyenPage.jsx`, `VueGenerale.jsx`, `ZonesRisquePage.jsx`, `AnomalyDashboard.jsx`, `SimulateurRisque.jsx`, `MlInsights.jsx`, `SegmentationPage.jsx`, `SatisfactionPage.jsx`), sans fichier de configuration centralisé ni variable d'environnement (`.env`) observée dans le code lu.
- **Fichiers `.bak` laissés dans le dépôt** : au moins 12 fichiers `.jsx.bak` coexistent avec leurs versions actives (`ZonesRisquePage`, `SimulateurRisque`, `AnomalyDashboard`, `MlInsights`, `SegmentationPage`, `LoginDecideurs`, `EvolutionTemporelle`, `MapView`, `ProtectedRoute`, `VueGenerale`, `WelcomePage`, `FormPage`). Au moins deux comparaisons concrètes confirment que ces fichiers sont des versions **antérieures non nettoyées** : `ProtectedRoute.jsx.bak` utilise des couleurs RGBA codées en dur au lieu des variables de thème (antérieur à l'introduction du dark/light mode), et `WelcomePage.jsx.bak` ne contient pas la fonctionnalité "Cellule d'écoute" (antérieur à l'ajout de la page `/avis`).
- **Style entièrement inline** : aucun composant n'utilise de CSS Modules ou de styled-components pour les pages décideurs — tout est `style={{...}}`, ce qui rend les fichiers JSX très longs (300 à 700+ lignes) et difficile à maintenir.
- **Logique métier mélangée à la présentation** : par exemple, `genererRecommandations()` dans `ZonesRisquePage.jsx` génère du texte métier directement dans le composant React, au lieu d'être une fonction utilitaire testable séparément.
- **Mots de passe en clair côté serveur** : le token JWT est désormais signé HS256 et vérifié côté serveur (non falsifiable sans `JWT_SECRET`), mais `DECIDEURS_DB` stocke encore les mots de passe en texte brut, sans hashing, et il n'existe pas de refresh token ni de révocation serveur.
- **`role` non utilisé fonctionnellement** dans `LoginDecideurs.jsx` : un state `role` existe et un tableau `roles` est affiché visuellement, mais la sélection ne joue aucun rôle dans le flux de connexion réel — c'est purement informatif/UI.
- **Anomalie d'encodage** observée dans `FormPage.css` (caractère non-ASCII inséré dans une valeur de `padding`), signe probable d'un copier-coller depuis une source externe sans nettoyage.
- **Pas de gestion d'erreur différenciée** : toutes les erreurs réseau affichent un message générique invitant à vérifier que FastAPI tourne sur le port 8000, sans distinction entre erreur réseau, 404, 500, ou timeout.
- **Recommandations en partie "fake-statiques"** : plusieurs pages (Anomalies, Zones à risque, Évolution temporelle) présentent des "recommandations" qui sont en réalité des textes pré-écrits côté frontend interpolés avec quelques vraies valeurs, et non une sortie générée dynamiquement par un système de règles ou d'IA.

### 12.3 Pistes d'amélioration (déduites des limites ci-dessus)
- Centraliser l'URL de l'API dans un seul fichier de configuration (ou variable d'environnement CRA `REACT_APP_API_URL`).
- Nettoyer le dépôt des fichiers `.bak` (ou les déplacer dans un historique Git plutôt que de les laisser dans `src/`).
- Migrer le style inline vers CSS Modules ou une librairie de composants pour réduire la taille des fichiers JSX.
- Hasher les mots de passe de `DECIDEURS_DB` (ex. `passlib`/`bcrypt`) et ajouter un refresh token / une révocation serveur, le backend d'authentification JWT de base étant désormais en place.
- Différencier les messages d'erreur réseau selon le code de statut HTTP retourné par `fetch`.

---

## 13. Questions possibles en soutenance — synthèse complémentaire

🎓 **Q1 — "Pourquoi le state du formulaire usager (`formData`) est-il dans `App.js` et pas dans `FormPage.jsx` lui-même ?"**
*Courte* : Parce que `App.js` orchestre l'appel API et doit garder le contrôle du cycle de vie de la requête.
*Détaillée* : `FormPage.jsx` est un composant purement présentationnel recevant `formData`, `handleChange`, `handleSubmit`, `loading`, `erreur`, `resultat`, `activeTab`, `resetForm` en props. C'est un pattern "smart parent / dumb child" : `App.js` reste responsable de la communication avec le backend (`fetch` vers `/recommander`) et de la gestion des états globaux liés à cette requête, tandis que `FormPage.jsx` ne gère que l'état purement local d'interface (`currentStep` du wizard).

🎓 **Q2 — "Comment est garantie la cohérence entre le rôle de l'utilisateur connecté et les modules visibles dans la sidebar ?"**
*Courte* : Par un double filtrage : `canAccess()` dans `auth.js` pour la logique RBAC, appliqué à la fois au niveau du routage (`ProtectedRoute`) et au niveau de l'affichage des menus (`DecideursLayout`, `VueGenerale`).
*Détaillée* : `auth.js` stocke `allowedRoutes` par utilisateur. `canAccess(session, subPath)` vérifie l'inclusion. `DecideursLayout.jsx` filtre chaque item de `NAV_GROUPS` via cette fonction pour griser les modules non autorisés. `VueGenerale.jsx` fait de même via sa propre fonction `hasAccess(path)` pour cacher entièrement certaines cartes KPI/modules. Il n'y a donc pas de source unique de vérité visuelle, mais une réplication cohérente de la même logique `canAccess`/`allowedRoutes` à plusieurs endroits.

🎓 **Q3 — "Le simulateur de risque utilise-t-il le même modèle que les autres pages ML ?"**
*Courte* : Le code ne permet pas de l'affirmer avec certitude — il appelle un endpoint différent (`/predict-inaccessibility`) de celui utilisé par `MlInsights.jsx` (`/api/ml/metrics`, `/api/ml/features-importance`).
*Détaillée* : Information non disponible dans le code source fourni quant à l'identité exacte du modèle backend derrière `/predict-inaccessibility` par rapport à celui dont les métriques sont affichées dans `MlInsights.jsx`. Le frontend ne fait aucune supposition explicite à ce sujet — il consomme simplement deux endpoints distincts sans lien de code direct entre eux.

🎓 **Q4 — "Comment le frontend sait-il distinguer une donnée réelle d'une donnée simulée à l'affichage ?"**
*Courte* : Uniquement via des commentaires/labels statiques écrits à la main, pas via un champ de métadonnée retourné dynamiquement par une API.
*Détaillée* : Dans `evolutionData.js`, les objets `SOURCE_LABEL_EMD` et `SOURCE_LABEL_TRAFIC` sont des dictionnaires statiques `{annee: label}` codés en dur, utilisés pour afficher des légendes ("EMD réelle (2015)", "Trafic réel (2019)", "Données simulées") dans `EvolutionTemporelle.jsx`. Il n'y a aucun mécanisme dynamique consultant le backend pour déterminer la nature réelle/simulée d'une donnée — cette information est entièrement pré-déterminée et fixée dans le code frontend.

🎓 **Q5 — "Qu'arrive-t-il si le backend FastAPI n'est pas démarré quand on navigue dans l'espace décideurs ?"**
*Courte* : La connexion elle-même échoue, puisque `login()` appelle désormais `POST /auth/login` ; sans backend actif, aucun JWT ne peut être obtenu et l'espace décideurs reste inaccessible.
*Détaillée* : Depuis l'introduction du JWT, `auth.js` n'est plus simulé en local : `login()` fait un appel réseau réel vers FastAPI. Si le serveur n'est pas démarré, cet appel échoue et `LoginDecideurs.jsx` affiche une erreur de connexion, avant même d'atteindre les routes protégées. Une fois connecté (backend actif au moment du login), si le serveur s'arrête ensuite, c'est au chargement des données métier (zones, anomalies, ML, segmentation, satisfaction) que les appels via `authFetch` échouent, déclenchant le `catch()` de chaque page qui affiche son bandeau d'erreur rouge avec un message tel que *« Le serveur FastAPI doit tourner sur le port 8000 pour charger le dashboard. »*

🎓 **Q6 — "Pourquoi `CockpitDakar.jsx` ne fait-il aucun appel `fetch` vers FastAPI ?"**
*Courte* : Parce que cette page délègue entièrement l'affichage des données BI à un outil externe (Knowage), consommé via iframe et non via API REST.
*Détaillée* : Le composant ne contient aucune logique de récupération de données JSON — il se contente de pointer une `<iframe>` vers le chemin proxifié `/knowage-vue/workspace/document-composite/Acceuil` et de manipuler le DOM de cette iframe pour simuler des clics d'onglets. Les données affichées dans le cockpit sont entièrement gérées côté Knowage (un outil BI tiers), hors du périmètre du code React documenté ici.

🎓 **Q7 — "Le système de session expire-t-il automatiquement pour tous les utilisateurs après le même délai ?"**
*Courte* : Oui, `SESSION_DURATION_MS` est une constante unique (1 heure) appliquée à tous les utilisateurs, indépendamment de leur rôle.
*Détaillée* : la durée de session (60 minutes) est désormais portée par le champ `exp` du JWT, calculé côté serveur (`JWT_EXPIRE_MIN = 60` dans `main.py`) au moment de `create_access_token`, plutôt que par une constante locale dans `auth.js`. Il n'existe aucune différenciation de durée de session selon le rôle `planificateur` ou `exploitation` dans le code lu.

🎓 **Q8 — "Comment l'application gère-t-elle le responsive / mobile ?"**
*Courte* : Principalement via des règles `@media (max-width: 768px)` dans `App.css`, et un état `mobileOpen` pour la sidebar de `DecideursLayout.jsx`.
*Détaillée* : `App.css` contient des règles responsives ciblant probablement le menu et les grilles de cartes (observées comme `@media max-width: 768px` dans le fichier). `DecideursLayout.jsx` gère un état `mobileOpen` pour un menu hamburger mobile distinct du mode `collapsed` (réduction volontaire sur desktop). Les autres pages (graphiques recharts en `gridTemplateColumns: '1fr 1fr'` fixes) ne semblent pas avoir de logique responsive spécifique observée au-delà de ce qui est défini globalement.

🎓 **Q9 — "Qu'est-ce qui distingue le rôle `planificateur` du rôle `exploitation` concrètement dans l'interface ?"**
*Courte* : Principalement l'accès au module `ml-insights` (audit du modèle ML), réservé au `planificateur`, et l'ordre de priorité des recommandations affichées dans `SatisfactionPage.jsx`.
*Détaillée* : Dans `auth.js`, seul l'utilisateur `u1` (planificateur) a `ml-insights` dans `allowedRoutes` ; `u2` (exploitation) ne l'a pas, ce qui le fait disparaître de la sidebar (`DecideursLayout`) et des modules de `VueGenerale`. De plus, `SatisfactionPage.jsx` réordonne les recommandations selon `ROLE_PRIORITY` (planificateur → priorité à "Infrastructure piétonne"/"Inclusion sociale" ; exploitation → priorité à "Sécurité"/"Qualité de service"), bien que les données sous-jacentes restent identiques pour les deux rôles.

🎓 **Q10 — "Le frontend valide-t-il les données du formulaire avant de les envoyer au backend ?"**
*Courte* : De façon limitée — principalement via du masquage conditionnel de champs (ex. cacher revenu/permis pour les mineurs), pas via une validation de schéma stricte avant l'envoi.
*Détaillée* : Dans `FormPage.jsx`, la logique observée masque certains champs selon l'âge ou le statut étudiant, et `App.js` force certaines valeurs (`revenu`/`nb_vehicules`/`permis` à 0/2) avant l'envoi pour les mineurs/étudiants. Pour `AvisCitoyenPage.jsx`, la seule validation observée est `valid = note > 0 && mode !== ''`, désactivant le bouton d'envoi si cette condition n'est pas remplie. Aucune validation de format plus poussée (regex, longueur minimale autre que `commentaire` à 600 caractères max) n'a été trouvée dans le code lu.

---

*Fin du document — Section Frontend React. Toutes les informations ci-dessus sont issues d'une lecture intégrale des fichiers listés en introduction, y compris leurs versions `.bak` lorsque pertinentes pour la comparaison.*
