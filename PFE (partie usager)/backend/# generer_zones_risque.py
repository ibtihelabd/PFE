# generer_zones_risque.py  → à mettre dans le même dossier que main.py
import pandas as pd
import numpy as np
import joblib
import json
...
# Reconstruire les features (même logique que ml_inaccessibilite.py)
...
# Charger le modèle et l'imputer sauvegardés
model   = joblib.load(os.path.join(MODELS_DIR, "inacc_model.pkl"))
imputer = joblib.load(os.path.join(MODELS_DIR, "inacc_imputer.pkl"))

df['prob_risque'] = model.predict_proba(imputer.transform(X))[:, 1]
...
zone_risk = (df.groupby('I2')
               .agg(
                   ...
                   enclavement=('score_enclavement', 'mean'),
